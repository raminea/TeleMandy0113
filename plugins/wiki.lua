...
function Wikipedia:wikintro(text, lang)
  ...
  if result and result.query then
    ...
    if page and page.extract then
      ...
      local text = "⚠️جست و جویی برای گزینه پیدا نشد "..text
      text = text..'\n'..table.concat(wikiusage, '\n')
      return text
    end
  else
    return "‼️ببخشید مشکلی رخ داده"
  end
end
...
function Wikipedia:wikisearch(text, lang)
  ...
  if result and result.query then
    local titles = ""
	 ...
	 titles = titles ~= "" and titles or "⚠️هیچ جوابی پیدا نشد"
	 return titles
  else
    return "متاسفم مشکلی رخ داده است‼️"
  end
...
local function run(msg, matches)
  ...
  if matches[1] == "look" then
    search = true
	 term = matches[2]
	 lang = nil
  elseif matches[2] == "look" then
    search = true
	 term = matches[3]
	 lang = matches[1]
  else
